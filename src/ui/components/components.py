...
def render():
    ...
    with gr.Column():
        ...
        def update_action_dropdown(category):
            ...
            print("Updating actions for category", category, ":", actions)
        ...
        def select_step(steps_table_value):
            print("Steps table value:", steps_table_value)
        ...
        def delete_step(workflow_id, selected_index):
            try:
                ...
                workflow_data = WorkflowService.get_workflow(workflow_id)
                if selected_index >= len(workflow_data.get("steps", [])):
        ...
        def on_workflow_select(workflow_id):
            try:
                ...
                if not workflow_data:
                    return None, "工作流不存在", [], "", ""
                steps = [[step["action_name"], json.dumps(step["params"])] for step in workflow_data.get("steps", [])]
                print("Loaded workflow:", workflow_data)
                return (
                    workflow_id,
                ...
                print("Error in on_workflow_select:", str(e))
                return None, str(e), [], "", ""
        ...
        def import_workflow(file):
            try:
                ...
                steps = [[step["action_name"], json.dumps(step["params"])] for step in workflow_data.get("steps", [])]
                print("Imported workflow:", workflow_data)
                return (
                    workflow_data["id"],
                    json.dumps(workflow_data, indent=2),
                    steps,
                ...
                print("Error in import_workflow:", str(e))
                return None, str(e), [], "", ""
